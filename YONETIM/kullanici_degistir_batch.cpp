...
{
    SET_NAME_OF_RUN_BATCH_BUTTON ( tr("Kullanıcı Değiştir")      );
    SET_FIRST_FOCUS_WIDGET       ( m_ui->lineEdit_kullanici_kodu );
    REGISTER_BUTTON_WIDGET       ( m_ui->widget_batch_buttons    );
    SET_ENTER_KEY_FOR_RUN_BATCH  ( true  );
    SET_AUTO_EXIT_BATCH ( true );

    SET_PAGE_TITLE    ( tr ( "KULLANICI DEĞİŞTİRME EKRANI" ) ) ;
...
int KULLANICI_DEGISTIR_BATCH::CHECK_VAR ( QObject * p_object )
{
    ...
    if ( p_object EQ m_ui->lineEdit_kullanici_kodu ) {
        ...
        if ( sql_query.SELECT() EQ 0 ) {
            MSG_ERROR(tr ( "Kullanıcı bulunamadı" ) , m_ui->lineEdit_kullanici_kodu );
        ...
        if ( m_kullanici_id NE 1 )  {
            ...
            if ( sql_query.SELECT() EQ 0 ) {
                MSG_ERROR( QObject::tr("Kullanıcı bilgisayarda kayıtlı değil.") , m_ui->lineEdit_kullanici_kodu );
                return ADAK_FAIL_UNDO;
            }
        }
...
int KULLANICI_DEGISTIR_BATCH::CHECK_RUN()
{
    ...
    if ( m_kullanici_id < 1 ) {
        MSG_ERROR(tr ( "Geçerli bir kullanıcı seçilmemiş" ) , m_ui->lineEdit_kullanici_kodu );
        return ADAK_FAIL;
    }
    ...
    if ( sql_query.SELECT() EQ 0 ) {
        m_ui->lineEdit_kullanici_parola->clear();
        MSG_ERROR(tr ( "Hatalı parola" ) , m_ui->lineEdit_kullanici_parola );
        return ADAK_FAIL;
    }
...
void KULLANICI_DEGISTIR_BATCH::RUN_BATCH()
{
    ...
    *m_kullanici_id_ptr = m_kullanici_id;
    ...
    if (sql_query.SELECT() EQ 0 AND m_kullanici_id NE 1) {
        MSG_ERROR(tr ( "Seçili kullanıcının bu veritabanında çalışma izni yok.\nYeni veritabanı seçiniz." ) , NULL);
        if ( VERITABANI_DEGISTIR(&DB,this) EQ -1 ) {
            exit(88);
        }
        MSG_INFO(tr("Firma değişikliği başarılı. \nSeçilen firma: "+ VERITABANI_ISMI().toUtf8() + " " + VERITABANI_TANIMI().toUtf8() ),NULL);
    }
}
