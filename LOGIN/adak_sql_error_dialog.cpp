                                   ...
                                   const QString& aciklama,    const QString& hatali_query )
{
    ...
    if ( G_FORM_NAME.size() > 0 ) {
        ...
        hatanin_olustugu_form = QObject::tr("KERNEL dışı ekran");
    }
...
void ADAK_SQL_ERROR_DIALOG::SLOT_HATA_MAILI_AT()
{
    if ( m_ui->text_edit_detayli_bilgi->toPlainText().isEmpty() EQ true ) {
        ADAK_ERROR("Lütfen \"Hatayı <b>NEREDE ve NE YAPARKEN</b> aldınız\" kısmını doldurunuz.\n Hatanın <b>Çözümü için Çok Önemlidir</b>.",m_ui->text_edit_detayli_bilgi,this);
        return;
    }

    if ( m_ui->line_edit_email->text().isEmpty() EQ true ) {
        ADAK_ERROR("Lütfen iletişim için e-posta adresinizi yazınız.",m_ui->line_edit_email,this);
        return;
    }

    if ( m_ui->line_edit_email->text().contains("@",Qt::CaseInsensitive ) EQ false ) {
        ADAK_ERROR("Lütfen geçerli bir e-posta adresi yazınız.",m_ui->line_edit_email,this);
        return;
    }
    ...
    QString gosterilecek_msg= QObject::tr( "Zaman ayırıp hatayı bildirdiğiniz için teşekkür ederiz.");
